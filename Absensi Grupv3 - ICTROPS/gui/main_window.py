...
class SmartAttendanceApp(ctk.CTk):
    ...
    def tampilkan_database(self):
            ...
            try:
                # Cek dengan try-except yang lebih aman
                if (self.db_window is not None and 
                    hasattr(self.db_window, 'window') and 
                    self.db_window.window.winfo_exists()):
                    
                    self.db_window.window.lift()
                    self.db_window.window.focus()
                    return
            except:
                # Jika ada error, reset dan buat baru
                self.db_window = None
